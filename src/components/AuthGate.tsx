...
type Access = 'checking' | 'pending' | 'revoked' | 'approved'
 ...
 * but pending → an "awaiting approval" screen; revoked → an "access revoked"
 * screen; approved → the app.
 */
export function AuthGate({ children }: { children: ReactNode }) {
    ...
    const applySession = (s: Session | null) => {
      ...
      getUserAccess(s.user.id)
        .then((a) => {
          ...
          lastResolved = true
          setAccess(a?.status === 'approved' ? 'approved' : a?.status === 'revoked' ? 'revoked' : 'pending')
        })
        .catch(() => {
          if (cancelled || my !== gen) return
  ...
  if (access === 'revoked') return <AccessRevoked email={session.user.email ?? 'your account'} />
...
/**
 * Signed in, but an admin has revoked access. Deliberately worded differently
 * from PendingApproval — telling someone whose access was withdrawn that they
 * are "awaiting approval" invites them to keep waiting for something that is
 * not coming.
 */
function AccessRevoked({ email }: { email: string }) {
  return (
    <div className="flex items-center justify-center min-h-screen bg-[var(--surface-2)] px-4">
      <div className="w-full max-w-[400px] bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 text-center shadow-[0_24px_60px_rgba(0,0,0,0.12)]">
        <div className="mx-auto mb-4 w-11 h-11 rounded-full bg-[var(--neg-surface)] border border-[var(--neg-border)] flex items-center justify-center">
          <Ban size={18} className="text-[var(--neg)]" />
        </div>
        <h1 className="font-display text-[18px] text-[var(--ink)] mb-2">Access revoked</h1>
        <p className="text-[13px] text-[var(--text-2)] leading-relaxed mb-1">
          You're signed in as <span className="font-semibold text-[var(--ink)]">{email}</span>.
        </p>
        <p className="text-[13px] text-[var(--text-2)] leading-relaxed mb-6">
          Your access to this dashboard has been removed. Contact an
